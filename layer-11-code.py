...
train = pd.read_csv('/kaggle/input/layer-11/train.csv')
valid = pd.read_csv('/kaggle/input/layer-11/valid.csv')
test = pd.read_csv('/kaggle/input/layer-11/test.csv')
...
# %%
for i in range(4):
    X_VALID[i] = scaler.fit_transform(X_VALID[i])
...
def svmModel(X_train,Y_train):
  ...
  model = SVC(kernel='rbf', random_state=42, C=102)
  Y_train_encoded = LE.fit_transform(Y_train[lable])
  # Train the model
  model.fit(X_train, Y_train_encoded)
  return model
...
def evaluator(X_train,Y_train,X_valid,Y_valid,X_test,model,y_lable="1" ):
  ...
  print(f"Valid Data Accuracy for y{y_lable}: {accuracy:.5f}")
...
for i in range(4):
  print(f"y{i+1} :")
  model_svm=svmModel(X_TRAIN[i],Y_TRAIN[i])
  y_pred_after=evaluator(X_TRAIN[i],Y_TRAIN[i],X_VALID[i],Y_VALID[i],X_test,model_svm, f"{i + 1}")
...
# %%
# Assuming y_pred is a torch tensor with shape (batch_size, num_classes)
_, predicted_class = torch.max(y_pred, dim=1)  # Get the index of the maximum value as the predicted class

# Now, predicted_class contains the predicted class for each sample
# It will have the shape (batch_size,)
# You can convert it to a (batch_size, 1) tensor if needed
predicted_class = predicted_class.unsqueeze(1)

print(predicted_class[:5])
print(Y_VALID[1][:5])

 # Evaluate the y1 using valid data
accuracy = accuracy_score(Y_VALID[1], predicted_class)
print(f"Valid Data Accuracy for y: {accuracy:.2f}")
...
def createCSVOutput(X_test_predict_array):
  ...
  filename = f'/kaggle/working/190420V_layer_11_results.csv'
